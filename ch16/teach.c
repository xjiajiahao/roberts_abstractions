...
* File: teach.c
* -----------------
* This program executes a simple programmed instruction course. * The course is specified by a data file containing all the
...
* Type: answerT
* --------------------
* This structure provides space for each possible answer * to a question.
*/
...
* needed to store one of the individual question records. * Because this structure is large ad it makes sense
...
* type: courseDB
* --------------------
* This type is used to define the entire database, which is * a pointer to a record containing the title and an array of * questions.
*/
...
static bool ReadOneQuestion (FILE *infile, courseDB course); static void ReadQuestionText (FILE *infile, questionT q);
static void ReadAnswers (FILE *infile, questionT q);

static FILE *OpenUserFile (string prompt, string mode); static void ProcessCourse (courseDB course);
...
* Usage: ReadDataBase ();
* ----------------------------------
* This function asks the user for a file name and reads * in the database for the course. The file is formatted * as discussed in the section "Designing the external
...
* read successfully, this function returns TRUE. When the end * of the file is encountered, the function returns FALSE.
...
static bool ReadOneQuestion (FILE *infile, courseDB course) {
    ...
    question = New (questionT); ReadQuestionText (infile, question);
...
* question data structure which must have been allocated * by the caller. The end of the question text is signaled
...
* followed by a colon, followed by the number of the next * question to be read. The end of the answer list is
...
* the answer in the database, and (d) goes to a new question * on the basis of that answer. In this implementation, the
...
* Usage: AskQuestion (q);
* --------------------------------
* This function asks the question indicate by the questionT * specified by q. Asking the question consists of displaying * each of the lies that comprise the question text.
*/
...
* -------------------------------------
* This function looks up the string ans in the list of answers * for question q. If the answer is found, its index in the
* answer list is returned. If not, the function returns –1.
* The function uses a simple linear search algorithm to look * thought the array .
*/
